...
class EE_Stripe_OAuth_Form extends EE_Form_Section_Proper
{
     ...
     * @var EE_PMT_Base
     */
    protected $_the_pm_instance = null;
     ...
     * @var string
     */
    protected $_connect_btn_text = null;
     ...
     * @var string
     */
    protected $_connect_btn_sandbox_text = null;
     ...
     * @var string
     */
    protected $_connected_sandbox_text = null;
     ...
     */
    function __construct(EE_PMT_Base $pmt, EE_Payment_Method $payment_method)
    {
        $this->_pmt = $pmt;
    ...
    {
        $field_heading = EEH_HTML::th(sprintf(
            esc_html__('Stripe Connect: %1$s *', 'event_espresso'),
            $this->_pmt->get_help_tab_link()
        ));

        // Is this a test connection?
        $livemode = $this->_the_pm_instance->get_extra_meta('livemode', true);
        $livemode_txt = (! $livemode) ? ' ' . EEH_HTML::strong(
        ...
        $subsactions['stripe_connect_btn'] = new EE_Form_Section_HTML(
            EEH_HTML::tr(
                $field_heading .
                EEH_HTML::td(
                    EEH_HTML::link(
        ...
        $subsactions['stripe_disconnect_btn'] = new EE_Form_Section_HTML(
            EEH_HTML::tr($field_heading .
                EEH_HTML::td(
                    EEH_HTML::img(
    ...
    public function enqueue_js()
    {
        $stripe_connect_args = array(
            ...
            'blocked_popups_notice'     => esc_html__('The authentication process could not be executed. Please allow window pop-ups in your browser for this website in order to process a successful authentication.',
                'event_espresso'),
        ...
        // Styles
        wp_enqueue_style('eea_stripe_connect_form_styles',
            EE_STRIPE_URL . 'assets' . DS . 'css' . DS . 'espresso_stripe_connect.css', array(),
            EE_STRIPE_VERSION);
        // Scripts
        wp_enqueue_script('eea_stripe_connect_form_scripts',
            EE_STRIPE_URL . 'assets' . DS . 'scripts' . DS . 'espresso_stripe_connect.js', array(),
            EE_STRIPE_VERSION);
